...
void solve()
{
      ...
      for (ll tt = 1; tt <= t1; tt++) {
            ...
            ll rinc[n]; rinc[n-1] = n-1;
            ll ldec[n]; ldec[n-1] = n-1;
            for(ll i = n - 2; i >= 0; i--){
                  if(a[i] <= a[i + 1])rinc[i] = rinc[i + 1]; else rinc[i] = i;
                  if(a[i] >= a[i + 1])ldec[i] = ldec[i + 1]; else ldec[i] = i;
            }
            vector<ll> ans;
            ll x = rinc[0]; ans.pb(x);
            x = ldec[x]; ans.pb(x);
            x = rinc[x];
            reverse(a + ans[0], a + ans[1] + 1);
            bool ok = 1;
            f(i,1,n) if(a[i] < a[i - 1]) ok = 0;
            if(ok)
